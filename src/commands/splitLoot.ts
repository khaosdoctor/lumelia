...
export async function splitLootCommand (ctx: BotContext) {
  const loadingMessage = await ctx.reply('Splitting loot... One moment please 🤖')
  const sessionText = ctx.message?.text?.split('/splitloot')[1]
  if (!sessionText) {
    return ctx.api.editMessageText(ctx.chat?.id!, loadingMessage.message_id!, 'No session text provided, please send the session text after the command 😊')
  }
  ...
  try {
    ...
    for (const transaction of transactions) {
      ...
      const outstandingBalance = getOutstandingBalance(session, maybePlayer, maybeReceiver) || new Balance(maybePlayer, maybeReceiver)
    ...
    ctx.deleteMessage()
    return ctx.api.editMessageText(ctx.chat?.id!, loadingMessage.message_id!, Array.from(balanceText.values()).join('\n'), {
      parse_mode: 'MarkdownV2'
    })
  } catch (error) {
    console.error(`Error splitting loot`, { error })
    return ctx.api.editMessageText(ctx.chat?.id!, loadingMessage.message_id!, `😞 Error splitting loot, please try again later`)
  }
}
