...
export function isTelegramUser (user: unknown): user is TelegramUser {
  return typeof user === 'object' && user !== null && 'userId' in user && 'name' in user
}
 ...
 */
export function findPlayerThatOwnsTheChar (session: BotSession, charName: CharName) {
 ...
 */
export function findCharsOwnedByPlayer (session: BotSession, player: TelegramUser) {
